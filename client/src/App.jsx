...
// ─── Helpers & shader code at module scope ────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const prog = new Program(gl, {
      ...
      uniforms: {
        ...
        uWarpAmplitude: { value: 65.0 },
        uBlendAngle: { value: 30.0 },
        uBlendSoftness: { value: 0.1 },
        uRotationAmount: { value: 380.0 },
        uNoiseScale: { value: 2.2 },
        uGrainAmount: { value: 0.055 },
        uGrainScale: { value: 2.5 },
    ...
    const loop = (t) => {
      prog.uniforms.iTime.value = (t - t0) * 0.001;
      ...
      {open && (
        <div className="crm-body">
          <div className="crm-row">
            <span>Status</span>
            <span className="crm-chip">{ticket.status}</span>
          </div>
          <div className="crm-row">
            <span>Created</span>
          ...
          <div>
            <p className="crm-label">Summary</p>
            <p>{ticket.summary}</p>
          </div>
        </div>
      )}
...
function PinUpload() {
  ...
  async function uploadFile(file) {
    ...
    try {
      ...
      setShowToast({
        ok: true,
        msg: `${data.file} · ${data.chunks} chunks ingested`,
      });
    } catch (err) {
...
function SoloChat({ onBack }) {
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "Welcome. I'm your document intelligence assistant — powered by RAG. Upload files or ask me anything about your ingested documents.",
      sources: [],
      conflicts: [],
      crmTicket: null,
    },
...
export default function App() {
  ...
  const [screen, setScreen] = useState("auth"); // 'auth' | 'lobby' | 'room' | 'solo'
  ...
  if (screen === "solo") return <SoloChat onBack={() => setScreen("lobby")} />;
}
